...
from services.utils.iam_token import get_iam_token
...
В поле summary напиши общее текстовое резюме дня, объединив все записи. Будь внимательным и заботливым, как психолог, который старается понять и поддержать. ВАЖНО: Пиши summary от второго лица или используй безличные конструкции. Описывай день так, как будто обращаешься непосредственно к автору записей. Длина summary не должна превышать 500 символов.
Если предоставленных текстов недостаточно для анализа, то в dominant_emotion укажи "neutral", а в summary - "Недостаточно данных для анализа". """
    ...
    def get_calendar_data(
        ...
        while current_date < end_date:
            ...
            calendar_data.append({
                "date": current_date,  # Важно: передаем объект date, а не строку
    ...
    def generate_daily_stats(self, user_id: int, target_date: date) -> Optional[DailyStats]:
        ...
        try:
            ...
            for record in records:
                # Считаем эмоции
                emotion_count[record.emotion] = emotion_count.get(
                    record.emotion, 0) + 1
            ...
            fallback_dominant_emotion = max(emotion_count.items(), key=lambda x: x[1])[
                0] if emotion_count else None
            ...
            if iam_token and all_summaries:
                try:
                    ...
                    logger.error(
                        f"Ошибка парсинга JSON от GPT: {str(e)}. Ответ: {gpt_response}")
                except Exception as e:
                    logger.error(
                        f"Ошибка при вызове GPT для анализа дневных записей: {str(e)}")
            ...
            logger.error(
                f"Error generating daily stats for user {user_id} on {target_date}: {str(e)}")
            self.db.rollback()
            return None
    ...
    def update_daily_summary(
        ...
        try:
            ...
            logger.error(
                f"Error updating daily summary for user {user_id} on {target_date}: {str(e)}")
            self.db.rollback()
            return None
