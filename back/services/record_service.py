...
class RecordService:
    ...
    def update_record(
        ...
        try:
            ...
            self.db.rollback()
            logger.error(f"Error updating record {record_id} for user {user_id}: {str(e)}")
            raise
    ...
    def delete_record(self, user_id: int, record_id: int) -> bool:
        try:
            ...
            self.db.rollback()
            logger.error(f"Error deleting record {record_id} for user {user_id}: {str(e)}")
            return False

    def get_user_records_stats(self, user_id: int) -> Dict[str, Any]:
        total_records = self.db.query(Record).filter(Record.user_id == user_id).count()
